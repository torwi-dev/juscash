import { IScraperRepository, CreateExecutionDto, UpdateExecutionDto, PaginationOptions } from '../domain/interfaces/IScraperRepository';
...
export class ScraperService {
  constructor(private scraperRepository: IScraperRepository) {}

  async createExecution(data: CreateExecutionDto): Promise<ScraperExecution> {
    // Validações de entrada
    if (!data.executionDate) {
      throw new ValidationError('Data de execução é obrigatória');
    }

    if (!(data.executionDate instanceof Date)) {
      throw new ValidationError('Data de execução deve ser um objeto Date válido');
    }
    ...
    if (data.djeUrl && !this.isValidUrl(data.djeUrl)) {
      throw new ValidationError('URL do DJE inválida');
    }

    if (data.hostName && data.hostName.length > 255) {
      throw new ValidationError('Nome do host deve ter no máximo 255 caracteres');
    }

    if (data.executedBy && data.executedBy.length > 100) {
      throw new ValidationError('Campo "executedBy" deve ter no máximo 100 caracteres');
    }

    if (data.environment && !['development', 'staging', 'production'].includes(data.environment)) {
      throw new ValidationError('Environment deve ser: development, staging ou production');
    }
    ...
    if (existingExecution) {
      throw new ExecutionConflictError(`Já existe execução para a data: ${data.executionDate.toISOString().split('T')[0]}`);
    }

    // Verificar se não há execução em andamento
    const runningExecution = await this.getRunningExecution();
    if (runningExecution) {
      throw new ExecutionConflictError(`Já existe execução em andamento (ID: ${runningExecution.id})`);
    }
  ...
  async updateExecution(id: number, data: UpdateExecutionDto): Promise<ScraperExecution> {
    if (!id || id < 1) {
      throw new ValidationError('ID de execução inválido');
    }
    ...
    if (existingExecution.status === 'completed' && data.status !== 'completed') {
      throw new BusinessRuleError('Execução concluída não pode ter status alterado');
    }

    if (existingExecution.status === 'failed' && data.status === 'running') {
      throw new BusinessRuleError('Execução falhada não pode voltar para "running"');
    }

    // Validações específicas por status
    if (data.status === 'completed') {
      if (data.publicationsFound === undefined || data.publicationsNew === undefined) {
        throw new ValidationError('Publicações encontradas e novas são obrigatórias para status "completed"');
      }

      if (data.publicationsFound < 0 || data.publicationsNew < 0) {
        throw new ValidationError('Número de publicações não pode ser negativo');
      }

      if (data.publicationsFound < data.publicationsNew) {
        throw new BusinessRuleError('Publicações novas não pode ser maior que publicações encontradas');
      }
    ...
    if (data.status === 'failed') {
      if (!data.errorMessage?.trim()) {
        throw new ValidationError('Mensagem de erro é obrigatória para status "failed"');
      }

      if (data.errorMessage.length > 1000) {
        throw new ValidationError('Mensagem de erro deve ter no máximo 1000 caracteres');
      }
    }

    // Validações de valores
    if (data.publicationsFound !== undefined && data.publicationsFound < 0) {
      throw new ValidationError('Publicações encontradas não pode ser negativo');
    }

    if (data.publicationsNew !== undefined && data.publicationsNew < 0) {
      throw new ValidationError('Publicações novas não pode ser negativo');
    }

    if (data.publicationsDuplicated !== undefined && data.publicationsDuplicated < 0) {
      throw new ValidationError('Publicações duplicadas não pode ser negativo');
    }

    // Validar tempo de execução
    if (data.endTime && existingExecution.startTime && data.endTime < existingExecution.startTime) {
      throw new BusinessRuleError('Data de fim não pode ser anterior ao início da execução');
    }

    return await this.scraperRepository.update(id, data);
  }

  async getExecutions(pagination: PaginationOptions = { page: 1, limit: 20 }) {
  ...
  async getExecutionById(id: number): Promise<ScraperExecution> {
    if (!id || id < 1) {
      throw new ValidationError('ID de execução inválido');
    }
  ...
  async getExecutionByDate(date: Date): Promise<ScraperExecution | null> {
  ...
  async completeExecution(id: number, publicationsFound: number, publicationsNew: number): Promise<ScraperExecution> {
    if (!id || id < 1) {
      throw new ValidationError('ID de execução inválido');
    }
    ...
    if (execution.status === 'completed') {
      throw new BusinessRuleError('Execução já está concluída');
    }

    if (execution.status === 'failed') {
      throw new BusinessRuleError('Execução falhada não pode ser concluída');
    }
  ...
  async failExecution(id: number, errorMessage: string): Promise<ScraperExecution> {
    if (!id || id < 1) {
      throw new ValidationError('ID de execução inválido');
    }

    if (!errorMessage?.trim()) {
      throw new ValidationError('Mensagem de erro é obrigatória');
    }
    ...
    if (execution.status === 'completed') {
      throw new BusinessRuleError('Execução concluída não pode ser marcada como falhada');
    }
  ...
  async getTodayExecution(): Promise<ScraperExecution | null> {
    const today = new Date();
  ...
  async getRunningExecution(): Promise<ScraperExecution | null> {
  ...
  async cancelExecution(id: number, reason: string): Promise<ScraperExecution> {
    if (!reason?.trim()) {
